...
.. module:: rotation_stages
   ...
   :synopsis: Hardware wrappers for rotation stages

.. moduleauthor:: Jonathan Wheeler <jamwheel@stanford.edu>

This module provides support for controlling rotation stages with Python. A
rotation stage can be imported by

>>> import hardware
>>> rot = hardware.rotation_stages.NSC_A1(port='COM3')
...
class NSC_A1:
    ...
    Hardware wrapper for NSC-A1 Single Axis USB Stepper Motor Controller

    The stepper motor is a USB to Serial interface. On Windows, you
    communicate with it over a COM port, where on Unix, you would
    communicate over a /dev/tt port.

    .. _NSC-A1: http://www.newmarksystems.com/motion-controllers/nsc-a1/

    Args:
        port (str): The serial port over which to communicate with the
            device.
        channel (int): Multiple axes can be controlled by the motor
        controller. Which channel is this rotational stage talking on?
        timeout (float): How many seconds should the serial interface wait
            for a response
            before timing out?
        max_x (int, float): The maximum x (in ticks) to rotate to.
            There are 10,000 ticks in one degree. Rotations beyond this
            bound will not occur and will raise exceptions
        min_x (int, float):
            The minimum x (in ticks) to rotate to. There are 10,000 ticks
            in one degree.
        verbose (bool):
    ...
    Attributes:
        x (int): The absolute position of the rotation stage in ticks
        angle (float): The absolute positon of the rotation stage in
            degrees
        max_x (int): The maximum position the stage can rotate to without
            raising an exception.
        min_x (int): The minimum posiiton the stage can rotate to without
            raising an exception
        max_angle (float): The maximum angle the stage can rotate to
            without raising an exception
        min_angle (float): The minimum angle the stage can rotate to
            without raising an exception
        velocity (float): The velocity of the stage in degrees per second.

    """
    def __init__(self, port='COM3', channel=1, timeout=0.1, max_x=30e4,
                 min_x=-30e4, verbose=False):
    ...
    def identify(self):
        """
        Returns:
            str: the results of the ``ID`` query.
        """
        model = str(self.cmd('ID'))
        return "Serial device on %s, %s" % (self.port, model)
    ...
    def _is_safe(self, destination_in_ticks):
        if (destination_in_ticks > self.max_x or
                destination_in_ticks < self.min_x):
            # return False
    ...
    def get_ticks(self, val, unit):
        """
        Converts a rotation distance into ticks.

        Args:
            val (float): The distance to rotate
            unit (str): The units, which can be 'deg', 'degree', 'degrees',
                'tick', 'ticks'

        Returns:
            float: The number of ticks
        ...
        if unit in ['deg', 'degree', 'degrees']:
            ...
            raise ValueError("I don't understand your unit {}".format(unit))
    ...
    def is_constant_speed(self, moving=False):
        """
        Args:
            moving (bool, optional): Only returns true if the stage is moving
               at a constant non-zero speed if true. If false, returns true if
               motor is moving constant speed (zero, or non-zero)

        Returns:
            bool: Whether the motor is moving at a constant speed

        """
        status = int(self.cmd('MST'))
        if not moving and status == 0:
            return True
        if status == 1:
            return True
        return False
    ...
    def is_stationary(self):
        """
        Returns:
            bool: True if the motor is stationary. False otherwise.
        """
        status = int(self.cmd('MST'))
        if status == 0:
            return True
